      ...
      {loading ? <p style={{ color: "#64748b" }}>Loading ferry manifests…</p> : manifests.length === 0 ? (
        ...
        <div style={{ overflowX: "auto" }}>
          ...
          {manifests.map((manifest) => (
            <div key={manifest.id} style={{ display: "grid", gridTemplateColumns: "1.5fr 1.2fr 1fr 0.8fr 1.5fr", gap: "0.75rem", minWidth: "760px", alignItems: "center", padding: "0.8rem 0", borderBottom: "1px solid #f1f5f9", fontSize: "0.86rem" }}>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", gap: "0.4rem" }}>
                {manifest.estado === "active" && <button type="button" className="pa-primary-btn" onClick={() => void openAdminEntryForm(manifest)} style={{ display: "inline-flex", alignItems: "center", gap: "0.3rem", whiteSpace: "nowrap" }}>Enter bookings</button>}
          ...
          {entryManifest && <section style={{ marginTop: "1.25rem", padding: "1rem", border: "1px solid #bfdbfe", borderRadius: "14px", background: "#f8fbff", minWidth: "760px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.75rem", marginBottom: "0.75rem" }}><div><h3 style={{ margin: 0, color: "#0f3d68" }}>Enter booking numbers</h3><p style={{ margin: "0.25rem 0 0", color: "#64748b", fontSize: "0.82rem" }}>Each saved booking is locked and the customer confirmation email is sent automatically when an email is available.</p></div><button type="button" className="pa-secondary-btn" onClick={() => setEntryManifest(null)}>Close</button></div>
            {adminEntries.map((entry) => { const draft = entryDrafts[entry.id] || { booking: "", receiver: "" }; return <div key={entry.id} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1.4fr 1.3fr auto", gap: "0.5rem", alignItems: "center", padding: "0.6rem 0", borderTop: "1px solid #e2e8f0" }}><strong>{entry.nombre_cliente}</strong><span>#{entry.numero_cuenta}</span>{entry.enviado_en ? <><span>{entry.numero_reserva}</span><span>{entry.nombre_receptor || "-"}</span><span style={{ color: "#166534", fontWeight: 700 }}>Saved</span></> : <><input className="pa-input" value={draft.booking} placeholder="Booking #" onChange={(event) => setEntryDrafts((current) => ({ ...current, [entry.id]: { ...draft, booking: event.target.value } }))} /><input className="pa-input" value={draft.receiver} placeholder="Receiver name" onChange={(event) => setEntryDrafts((current) => ({ ...current, [entry.id]: { ...draft, receiver: event.target.value } }))} /><button type="button" className="pa-primary-btn" disabled={!draft.booking.trim()} onClick={() => void saveAdminEntry(entry)}>Save</button></>}</div>; })}
          </section>}
        </div>
      )}
    </div>
