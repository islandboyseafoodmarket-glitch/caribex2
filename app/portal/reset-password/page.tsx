...
export default function CustomerResetPasswordPage() {
  ...
  if (!hasSession) {
    return <main className="portal-page portal-centered"><section className="portal-card portal-first-login"><Image src="/imagenes/logo.png" alt="Caribex Logistics Group" width={165} height={50} /><h1>Reset link unavailable</h1><p>This password-reset link may have expired or already been used. Request a new link from the customer portal.</p><Link href="/portal/login" className="portal-button">Return to customer portal</Link></section></main>;
  }

  return <main className="portal-page portal-centered"><section className="portal-card portal-first-login"><Image src="/imagenes/logo.png" alt="Caribex Logistics Group" width={165} height={50} /><p className="customer-login-eyebrow">Customer portal security</p><h1>Change your password</h1><p>Choose a new password for your Caribex customer portal.</p><div className="portal-password-row"><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="New password" autoComplete="new-password" /><input type="password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} placeholder="Confirm new password" autoComplete="new-password" /><button type="button" className="portal-button" disabled={saving} onClick={() => void savePassword()}>{saving ? "Saving…" : "Save new password"}</button></div>{error && <small className="portal-error">{error}</small>}{message && <><small className="portal-success">{message}</small><Link href="/portal/login" className="customer-login-back">Sign in to the customer portal</Link></>}</section></main>;
}
